...
import com.nickperov.stud.micro_notes_spring.service.NotesServiceImpl;
import java.net.URI;
...
public class NotesApplicationTest {
    ...
    public static void startApp() throws InterruptedException {
        new Thread(() -> Application.main(new String[0])).start();
        ...
        final var context = Application.getWebApplicationContext();

        // Configure moc MVC
        final DefaultMockMvcBuilder builder = MockMvcBuilders.webAppContextSetup(context);
        mockMvc = builder.build();

        notesService = context.getBean(NotesServiceImpl.class);
    }
