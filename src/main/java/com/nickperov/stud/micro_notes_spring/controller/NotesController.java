package com.nickperov.stud.micro_notes_spring.controller;

import com.nickperov.stud.micro_notes_spring.Application;
import com.nickperov.stud.micro_notes_spring.api.NoteDTO;
import com.nickperov.stud.micro_notes_spring.service.NotesService;
import java.net.URI;
...
public class NotesController {
    ...
    public List<NoteDTO> listNotes() {
        return notesService.getAllNotes().stream().sequential().map(NoteDTO::new).collect(Collectors.toList());
    }
    ...
    public ResponseEntity<?> updateNote(@PathVariable("note_id") final UUID noteId, @RequestBody NoteDTO note) {
        ...
        if (isUpdated) {
            ...
            final URI uri = ServletUriComponentsBuilder.fromCurrentRequestUri().port(Application.getPort()).build().toUri();
